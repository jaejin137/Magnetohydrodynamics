C======================================================================C
c This part is to enforced nrbc in j direction at maximum index side.  c
C======================================================================C
clist(1)
      subroutine sr_nrj(iq, kq, il, jl, kl, nl, x, y, z,
...
c
      ...
      ip = i + 1
c
c ... jacobian and primitive variables at maximum j index
c
      cf0 = 64.d0
c
      do 10 j = jl - 1, jl + 1
...
c
   ...
   10 continue
c
c ... compute metrics ( at max j boundary )
c
      j = jl
      jp = jl + 1
      xet = x(i,jp,k) -x(i,j,k) +x(ip,jp,k) -x(ip,j,k)+
...
c
      do 20 i = 1, 5
        pvpxi(i) = pv(i,3)/jcbn(3)-pv(i,2)/jcbn(2)
   20 continue
c
c ... compute xli, plpv, & dxi
c
      if ( bc_eta_upper(1,1).eq.14 ) then
        call gdwall(gamma, xix, xiy, xiz, jcb, bxi, pv,
     >              pvpxi, alf, xli, plpv, dxi, 2)
      end if
c
      if ( bc_eta_upper(1,1).eq.15 ) then
        call gdsdu(gamma, xix, xiy, xiz, jcb, bxi, pv,
...
C======================================================================C
clist(2)
      subroutine sl_nrj(iq, kq, il, jl, kl, nl, x, y, z,
     $                  ilower, iupper, jlower, jupper, klower, kupper,
     $                  bc_eta_lower, bc_eta_upper,
...
C======================================================================C
clist(3)
      subroutine gdwall(gamma, xix, xiy, xiz, jcb, bxi, pv, pvpxi,
     >                  alf, xli, plpv, dxi, iul)
c
c This subroutine is used to calculate vector d for no-slip wall.
c
      implicit none
c
c *** INTERFACE VARIABLES
c
      integer, intent(in)::
     $     iul                  ! =1: upper wall; =2 lower wall
c
      double precision, intent(in)::
     $     gamma, xix, xiy, xiz, jcb, bxi, pv(5,3), pvpxi(5), alf
c
      double precision, intent(out)::
     $     xli(5),              ! italic Ls in xi direction
     $     plpv(5,5),           ! pl/pv
...
c ... plpv(5,i)
c
        cf0 = bxi/rr/rr/1.4142136d0
        cf1 = cc/rr/1.4142136d0
        plpv(5,1) = cf0*pvpxi(5)-cf1*( dcpu+capu*amrp )
        plpv(5,2) = cf1*xix*amrp
        plpv(5,3) = cf1*xiy*amrp
        plpv(5,4) = cf1*xiz*amrp
        plpv(5,5) = 0.d0
c
c ... plpv(4,i)
c
...
c
        lmd = capc
        xli(4) = lmd*( pvpxi(5)/rr/cc+dcpu/bxi )/1.4142136d0
        xli(5) = xli(4)
c
c ... compute pl/pv
c
...
c
        cf0 = bxi/rr/rr/1.4142136d0
        cf1 = cc/rr/1.4142136d0
        plpv(4,1) = -cf0*pvpxi(5)-cf1*( dcpu+capu*amrp )
        plpv(4,2) = cf1*xix*amrp
        plpv(4,3) = cf1*xiy*amrp
        plpv(4,4) = cf1*xiz*amrp
        plpv(4,5) = 0.d0
c
c ... plpv(5,i)
c
...
c
      dxi(1) = cf2*aac*( xli(4)+xli(5) )
c
      cf0 = cf1*cf2*xix
      dxi(2) = cf0*( xli(4)-xli(5) )
c
      cf0 = cf1*cf2*xiy
      dxi(3) = cf0*( xli(4)-xli(5) )
c
      cf0 = cf1*cf2*xiz
      dxi(4) = cf0*( xli(4)-xli(5) )
c
...
C======================================================================C
clist(4)
      subroutine gdsdu(gamma, xix, xiy, xiz, jcb, bxi, pv, pvpxi,
     >                 alf, poutlet, mx2, xli, plpv, dxi)
c
c This subroutine is used to calculate vector d for sideflow at upper
...
c
        ...
        cfx = dcpu+cf0
        plpv(4,1) = ( pupv(1)*cfx+lmd*pfpv(1) )*cf1
        plpv(4,2) = ( pupv(2)*cfx+lmd*pfpv(2) )*cf1
        plpv(4,3) = ( pupv(3)*cfx+lmd*pfpv(3) )*cf1
        plpv(4,4) = ( pupv(4)*cfx+lmd*pfpv(4) )*cf1
        plpv(4,5) = ( pupv(5)*cfx+lmd*pfpv(5) )*cf1
c
c --- outside ---
c
        rr = pv(1,3)
